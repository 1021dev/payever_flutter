...
class Terminal{
  ...
  Terminal.toMap(dynamic obj){
    ...
    _channelSet     = obj[GlobalUtils.DB_POS_TERMINAL_CHANNELSET];  
    _createdAt      = obj[GlobalUtils.DB_POS_TERMINAL_CREATEDAT];  
    _defaultLocale  = obj[GlobalUtils.DB_POS_TERMINAL_DEFAULTLOCALE];  
    ...
    _updatedAt      = obj[GlobalUtils.DB_POS_TERMINAL_UPDATEDAT];  
    __v             = obj[GlobalUtils.DB_POS_TERMINAL_V];  
    _id             = obj[GlobalUtils.DB_POS_TERMINAL_ID];

    dynamic subs = obj[GlobalUtils.DB_POS_TERMINAL_INTEGRATIONSUB];
     subs.forEach((sub){
       _subscription.add(sub);
     });

    dynamic locales = obj[GlobalUtils.DB_POS_TERMINAL_INTEGRATIONSUB];
...
class ChannelSet{
  ...
  ChannelSet.toMap(dynamic obj){
    ...
    _checkout = obj[GlobalUtils.DB_POS_CHANNELSET_CHECKOUT];
    _id       = obj[GlobalUtils.DB_POS_CHANNELSET_ID];
    _name     = obj[GlobalUtils.DB_POS_CHANNELSET_NAME];
    _type     = obj[GlobalUtils.DB_POS_CHANNELSET_TYPE];
...
class Product{
  ...
  Product.toMap(dynamic obj){
    _channelSet = obj[GlobalUtils.DB_POS_TERM_PRODUCT_CHANNELSET];
    _id         = obj[GlobalUtils.DB_POS_TERM_PRODUCT_ID];
    _lastSell   = obj[GlobalUtils.DB_POS_TERM_PRODUCT_LASTSELL];
...
class Cart {
  ...
  static items2Map(List<CartItem> items) {
    ...
    items.forEach((f){
      result.add({ GlobalUtils.DB_POS_CART_CART_ID :f.id,
